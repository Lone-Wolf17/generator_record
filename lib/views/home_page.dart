...
import 'file:///C:/Users/Lone%20Wolf/AndroidStudioProjects/power_records/lib/views/my_drawer.dart';
  ...
  _storeToDB(PowerState newState) async {
    ...
    _endState(
      ...
      List<Map> list = await txn.rawQuery(
          "SELECT ${DbHelper.durationInMinsCol} FROM ${DbHelper.mainRecordTable}"
          " WHERE ${DbHelper.startDateTimeCol} = '${startTime.toIso8601String()}' "
      ...
      if (currentDateTime.day == startTime.day &&
        ...
        while (!currentDateTime.isSameDate(newStartDate) &&
          ...
          String queryStr =
              ...
              " WHERE ${DbHelper.startDateTimeCol} ='${newStartDate.toIso8601String()}'"
              " AND ${DbHelper.powerSourceCol} = '$stateStr'";

          var result = await txn.rawQuery(queryStr);

          if (result.isEmpty) {
            DateTime newEndDateTime = DateTime(newStartDate.year,
                newStartDate.month, newStartDate.day, 23, 59);
                              ...
                              child: InkWell(
                                ...
                                child: Text("NEPA",
                                    style: TextStyle(
                                        ...
                                        fontWeight: FontWeight.bold,
                                        color:
                                            currentPowerState != PowerState.Nepa
                                                ? Colors.grey
                                                : Colors.green)),
                              ),
                            ),
